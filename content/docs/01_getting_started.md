...
Chrome is the preferred browser for running Sakuli tests at the moment. A suitable WebDriver can be installed via: 
...
To avoid eventual installation problems for Windows users we recommend to first install Python 2 on your system separately by downloading the required version on the official web page. Afterwards you can install the Windows Build Tools manually or via npm using: 
...
In case of errors when installing the Windows Build Tools package via `npm`, please make sure that the PowerShell is available on your system `PATH`. Additionally, you should install the Windows Build Tools by using the PowerShell in administrative mode.  
...
To describe the testsuite with its testcases, two additional files are needed: `testsuite.properties` and `testsuite.suite`. These files are required for backwards compatibility (they might not be necessary in the future but will at least be supported). These files should be added to the `my-sut` folder.
...
This is the minimum configuration for using Sakuli. The `.properties` file adds some metadata for the Sakuli-Runtime and can further be changed to configure other things like forwarders or the default browser for the execution.
...
The actual testcase file must be placed inside a folder (this is due to the format which was forced by Sahi in Sakuli v1). The start-url also needs to be added but has no effect in v2+.
...
After this setup you can add the actual test code to `my-testcase/testcase.js`:
...
4. In some cases, it is really helpful to visually verify test execution. Sakuli comes with a built-in `_highlight` function, which will highlight an element with a bright red border. Although being useful, `_highlight` should be used carefully since it will increase the overall test runtime.
...
When installed with the additional flag `-g`, Sakuli will be installed and added to your system `PATH` (Windows users still have to add it manually):
